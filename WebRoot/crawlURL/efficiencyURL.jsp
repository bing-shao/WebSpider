...
<body class="right_body">
	...
	<div id="chart0" style="margin-top:20px;margin-left:4%;width:95%;height:400px"></div>
...
function Pie_char() {
	$.ajax({
		       type: "post",
		       url : "EfficiencyURLAction_efficiencyURL",
		       dataType : 'json',
		       data:{
		       		fn:"Pie_char"
		       },
			   ...
			   success : function(response) {
			   		...
			   		if(jsondata[0].numT!=""){
				   		var data =  [[ '网址抓取成功', parseInt(jsondata[0].numT)], [ '网址抓取失败', parseInt(jsondata[0].numF)]]; 
						var line_title; 
						var title="本次抓取效率统计图"; 
						var x; 
						var x_label; 
						var y_label; 
						var data_max; 
						j.jqplot.diagram.base("chart0", [ data ], line_title, title, x, x_label, y_label, data_max, 3);
					}
				}
			});
}
...
function Histogram() {
		$.ajax({
			   ...
			   success : function(response) {
			   			...
			   			var maxkey = 0; 
				   		var keys = new Array(parseInt(${urlThreadCount}));
						var values = new Array(parseInt(${urlThreadCount}));
